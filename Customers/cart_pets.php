...
<?php
 ...
 extract($_SESSION); 
		...
		<?php
 include("config.php");
		  $stmt_edit = $DB_con->prepare("select sum(adopt_total) as total from adoptdetails where user_id=:user_id and adopt_status='adopted'");
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_delete = $DB_con->prepare('DELETE FROM adoptdetails WHERE adopt_id =:adopt_id');
		$stmt_delete->bindParam(':adopt_id',$_GET['delete_id']);
		$stmt_delete->execute();
		
		header("Location: cart_pets.php");
	}
...
<?php
	...
	if(isset($_GET['update_id']))
	{
		...
		$stmt_delete = $DB_con->prepare('update adoptdetails set adopt_status="adopted" WHERE adopt_status="Pending" and user_id =:user_id');
		$stmt_delete->bindParam(':user_id',$_GET['update_id']);
		$stmt_delete->execute();
		echo "<script>alert('pet/s successfully adopted!')</script>";	
		
		header("Location: adopts.php");
	}
...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
         ...
         <center><h3> <span class="fa fa-cart-plus"></span> 認養清單</h3></center>
        </div>
...
include("config.php");
	...
	$stmt = $DB_con->prepare("SELECT * FROM adoptdetails where adopt_status='Pending' and user_id='$user_id'");
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			?>
                <tr>
                 ...
                 <td><?php echo $adopt_name; ?></td>
				 <td><?php echo $adopt_price; ?> </td>
				 <td><?php echo $adopt_quantity; ?></td>
				 <td><?php echo $adopt_total; ?> </td>
				 ...
				 <td>
                  ...
                  <a class="btn btn-block btn-danger" href="?delete_id=<?php echo $row['adopt_id']; ?>" title="click for delete" onclick="return confirm('確定要移除嗎?')"><span class='glyphicon glyphicon-trash'></span> 移除</a>
		 ...
		 include("config.php");
		  $stmt_edit = $DB_con->prepare("select sum(adopt_total) as totalx from adoptdetails where user_id=:user_id and adopt_status='Pending'");
		$stmt_edit->execute(array(':user_id'=>$user_id));
		$edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($edit_row);
		
		echo "<tr>";
		echo "<td colspan='3' align='right'>總金額:";
		echo "</td>";
		...
		echo "<td>";
		echo "<a class='btn btn-block btn-success' href='?update_id=".$user_id."' ><span class='glyphicon glyphicon-shopping-cart'></span> 確定認養!</a>";
		echo "</td>";
	...
	else
	{
        ...
        <div class="col-xs-12">
        	<div class="alert alert-warning">
            	<span class="glyphicon glyphicon-info-sign"></span> &nbsp; 尚未選擇寵物
            </div>
        </div>
        <?php
	}
