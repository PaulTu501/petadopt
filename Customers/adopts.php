...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
         ...
         <center><h3> <span class="glyphicon glyphicon-list-alt"></span> 已認養的寵物</h3></center>
        </div>
...
include("config.php");
	...
	$stmt = $DB_con->prepare("SELECT * FROM adoptdetails where adopt_status='adopted' and user_id='$user_id'");
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
		 ...
		 include("config.php");
		  $stmt_edit = $DB_con->prepare("select sum(adopt_total) as totalx from adoptdetails where user_id=:user_id and adopt_status='adopted'");
		$stmt_edit->execute(array(':user_id'=>$user_id));
		$edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($edit_row);
	...
	else
	{
        ...
        <div class="col-xs-12">
        	<div class="alert alert-warning">
            	<span class="glyphicon glyphicon-info-sign"></span> &nbsp; No pet Found ...
            </div>
        </div>
        <?php
	}
